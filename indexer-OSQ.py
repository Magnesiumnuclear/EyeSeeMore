...
# --- 設定區 ---
DB_FILE = "images.db"  # 修改：改為資料庫檔案名稱
    ...
    def __init__(self):
        ...
        try:
            ...
            if os.path.exists(DB_FILE):
                ...
                # 從資料庫讀取所有圖片路徑與向量
                # 假設 table 名稱是 images，欄位是 file_path (Text) 和 embedding (Blob)
                try:
                    ...
                    for path, blob in rows:
                        paths.append(path)
                        # 將 Blob 轉回 numpy array (float32)
                        # 注意：寫入時必須是 array.tobytes() 且 dtype 為 float32
                    ...
                    if paths:
                        self.stored_paths = paths
                        
                        # 將 List[numpy array] 堆疊成單一 Matrix (N, D)
                        emb_matrix = np.stack(embeddings_list)
                        
                        # 轉為 PyTorch Tensor 並移至 GPU/CPU
                        self.stored_embeddings = torch.from_numpy(emb_matrix).to(self.device)
                        
                        self.is_ready = True
                        print(f"[Engine] Successfully loaded {len(self.stored_paths)} records from database.")
                    else:
                        print(f"[Engine] Database is empty.")
